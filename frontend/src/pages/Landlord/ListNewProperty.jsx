...
export default function ListNewProperty() {
  ...
  const SubmitProperty = async(reactHookFormData) => {
    const imageUrl=await handlePropertyImagesUpload(images,reactHookFormData.title,session._id);
    if(imageUrl){
      reactHookFormData.propertyImage = imageUrl;
    }
    await postRequest(PropertyEndPoints.ListNewProperty,reactHookFormData);
  };

  return (
    <section className="max-w-[1290px] mx-auto my-6 px-4">
      <section className="flex flex-col gap-8">
        <h1 className="text-3xl">List Your New Property</h1>
        <form
                ...
                images?.map((img, index) => (
                  <section key={index} className="relative">
                    <img
                      ...
                      className="w-32 h-32"
                    />
                    <span className="absolute top-0 right-0 bg-white rounded-full" onClick={() => handleImageRemove(index)}>
                      <RxCrossCircled
                        className="text-3xl text-red-500 hover:scale-105 cursor-pointer"
                      />
                    </span>
                  </section>
                ))}

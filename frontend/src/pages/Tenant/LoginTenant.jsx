...
export default function LoginTenant() {
  ...
  const login=async(reactHookFormData)=>{
    const response=await postRequest(AuthEndPoints.LoginTenant,reactHookFormData);
    if(response){
  ...
  return (
    <section className="font-poppins px-4 py-4">
        ...
        <form onSubmit={handleSubmit(login)} className="w-[500px] h-[600px] flex flex-col justify-evenly px-4 py-4 shadow-lg rounded-md">
          <h1 className="text-xl">Welcome Back Tenant</h1>
          ...
          <section>
            <span className="text-gray flex gap-2">
                ...
                <span>
                    <Link className="text-blue-500 hover:underline" to={'/registertenant'}>
                        Register
                    </Link>
                </span>
            </span>

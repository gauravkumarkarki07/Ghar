...
export default function TenantPassword() {
  const session=useSelector(state=>state.Tenant.TenantSession.tenantDetails);
  ...
  const ChangePassword = async (reactHookFormData) => {
    const response=await putRequest(TenantEndPoints.ChangePassword,reactHookFormData);
    if(response){
      dispatch(logout());
    }
  };
